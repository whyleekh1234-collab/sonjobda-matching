...
-- ════════════════════════════════════════════════════════════
-- 12. 2단계 적용 후 수동으로 해야 하는 것
-- ════════════════════════════════════════════════════════════
--
-- 1) Authentication > Sign In / Providers > Email > "Confirm email"을 끈다.
--    켜져 있으면 회원가입 직후 세션이 없어 complete_signup 호출이 실패한다.
--    (관리자 승인 절차가 이미 있으므로 이메일 인증은 불필요하다.)
--
-- 2) 첫 계정을 하나 만든 뒤, 아래로 본인을 승인 + 운영자로 지정한다.
--    SQL Editor는 RLS를 우회하는 postgres 권한으로 실행되므로 이 두 줄만
--    직접 실행하면 된다. <이메일> 자리를 실제 가입 이메일로 바꾼다.
--
